...
class RNN(object):
    ...
    def __init__(self, summary_frequency, num_nodes, num_layers,
                 batch_generator, labeled=False, output_shape=2, only_retrain_output=False):
        ...
        self.w = tf.Variable(tf.truncated_normal([self.num_nodes, self.output_shape], -0.1, 0.1), name='output_w')
        self.b = tf.Variable(tf.zeros([self.output_shape]), name='output_b')

        if self.labeled:
            logits = tf.matmul(tf.concat(axis=0,values=outputs), self.w) + self.b
        else:
            logits = tf.matmul(tf.concat(axis=0,values=outputs[:-1]), self.w) + self.b

        sample_logits = tf.matmul(tf.concat(axis=0,values=outputs), self.w) + self.b
    ...
    def sample_distribution(self, distribution):
        ...
        r = random.uniform(0, np.sum(distribution))
        s = 0
    ...
    def return_state(self, text):

        assert len(text) >= self.num_unrollings, 'Input length needs to be >= ' + str(self.num_unrollings)

        with self.session.as_default():

            batches = self.sentence_to_batch(text)
            feed_dict = dict()
            for i in range(self.num_unrollings+1):
                    feed_dict[self.train_data[i]] = batches[i]


        return self.session.run([self.state], feed_dict=feed_dict)[-1][-1][-1]


    def return_state_per_letter(self, text):

        text = ' '*(self.num_unrollings+1)+text
        state_list = []
        cursor = 0
        while cursor < len(text)-(self.num_unrollings+1):
            temp_text = text[cursor:cursor+self.num_unrollings+1]
            print temp_text
            state_list.append(self.return_state(temp_text))
            cursor += 1
        return state_list
